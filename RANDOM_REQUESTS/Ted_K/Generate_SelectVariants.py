...
if __name__ == '__main__':
    ...
    tmp = []
    with open("//home/mccuem/shared/Projects/HorseGenomeProject/Data/ibio_EquCab3/ibio_output_files/GCF_002863925.1_EquCab3.0_genomic/GCF_002863925.1_EquCab3.0_genomic.fna.fai", "r") as input_f:
        for line in input_f:
            line = line.rstrip("\n").split("\t")
            a,b = line[0].split(".")
            c = a + "_" + b
    ...
    for i in tmp:
        ...
        with open(pbs, "w") as f:
            ...
            print(f"{s3cmd} get s3://mccue-lab/ibiodatatransfer2019/joint_gatk/" + i +".gvcf.gz\n"
                  + f"{s3cmd} get s3://mccue-lab/ibiodatatransfer2019/joint_gatk/" + i +".gvcf.gz\n" 
                  + f"gatk {java}"
                  + " SelectVariants -R "
                  + f"{ref} {select} "

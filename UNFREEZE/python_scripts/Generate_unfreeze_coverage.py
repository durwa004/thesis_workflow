...
if __name__ == '__main__':
    ...
    header = (
              ...
              f"#PBS -o $PBS_JOBID.s3cmd_{com}_{ft}.out\n"
              f"#PBS -e $PBS_JOBID.s3cmd_{com}_{ft}.err\n"
              f"#PBS -N s3cmd_{com}_{ft}.pbs\n"
              "#PBS -q small\n"
             )
    ...
    pbs = os.path.join(os.getcwd(),f"s3cmd_{com}_{ft}.pbs")
    
    with open(pbs, "w") as f:
        print(header, file=f)
        print(f"cd {data}\n", file=f)
        for i in tmp:
            print(f"{s3cmd} {com} s3://mccue-lab/ibiodatatransfer2019/{i}/metrics/{ft} {i}_{ft}",file=f) 

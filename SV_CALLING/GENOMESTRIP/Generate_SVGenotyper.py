...
if __name__ == '__main__':
    ...
    header = (
              ...
              f"#PBS -o $PBS_JOBID.{breed}.SVGenotyper.out\n"
              f"#PBS -e $PBS_JOBID.{breed}.SVGenotyper.err\n"
              f"#PBS -N {breed}.SVGenotyper.pbs\n"
              "#PBS -q small\n\n"
              "source ~/.genomestrip_bashrc"
             )
    
    pbs = os.path.join(os.getcwd(),f"{breed}.genomestrip.SVGenotyper.pbs")
    
    with open(pbs, "w") as f:
        print(header, file=f)
        print(f"cd {data}\n", file=f)
        print(f"runDir={data}/SVGenotyper", file=f)
        print("java -Xmx4g -cp ${SV_CLASSPATH} \\"
        ...
        + "    -vcf ${runDir}/ibio_" + breed + "_genomestrip.dels.vcf \\"
        + "    -O ${runDir}/ibio_" + breed + "_genomestrip.dels.genotypes.vcf \\"
        + "    -jobRunner Drmaa \\"
        + "      -gatkJobRunner Drmaa \\"
        + "      -jobQueue mcqueue \\"
        + "      -memLimit 10 \\"

...
if __name__ == '__main__':
    ...
    header = (
              ...
              f"#PBS -o $PBS_JOBID.{breed}.SVCNV_low.out\n"
              f"#PBS -e $PBS_JOBID.{breed}.SVCNV_low.err\n"
              f"#PBS -N {breed}.SVCNV_low.pbs\n"
              "#PBS -q small\n\n"
              "source ~/.genomestrip_bashrc\n"
              "export OMP_NUM_THREADS=1"
             )
    
    pbs = os.path.join(os.getcwd(),f"{breed}.genomestrip.SVCNV_low.pbs")
    
    with open(pbs, "w") as f:
        print(header, file=f)
        print(f"cd {data}\n", file=f)
        print(f"runDir={data}/SVCNV_low", file=f)
